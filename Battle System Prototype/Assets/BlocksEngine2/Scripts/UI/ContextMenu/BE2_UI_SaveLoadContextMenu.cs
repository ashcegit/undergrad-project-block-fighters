...
public class BE2_UI_SaveLoadContextMenu : MonoBehaviour, I_BE2_UI_ContextMenu
{
    ...
    public void Save()
    {
        StreamWriter sw = new StreamWriter(GetFullPath(), false);

        _targetProgrammingEnv.UpdateBlocksList();

        foreach (I_BE2_Block block in _targetProgrammingEnv.BlocksList)
        {
    ...
    public void Load()
    {
        ...
        {
            var sr = new StreamReader(GetFullPath());
            ...
            foreach (string xmlBlock in xmlBlocks)
            {
                ...
                BE2_BlocksSerializer.SerializableToBlock(serializableBlock, _targetProgrammingEnv);
            }
